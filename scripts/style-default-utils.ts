...
export function subjectForCategory(category: string) {
  ...
  if (lower.includes("portrait") || lower.includes("fashion") || lower.includes("costume")) return "a single adult model in a clean vertical editorial pose";
  if (lower.includes("film") || lower.includes("photography") || lower.includes("genre")) return "a vertical urban documentary scene with one clear subject, architecture, foliage, fabric, glass, and atmospheric depth";
  if (lower.includes("camera")) return "a vertical tabletop scene with objects at multiple scales, near detail, distant background, reflective surfaces, and geometric forms";
  if (lower.includes("lighting")) return "a vertical interior scene with one figure, textured wall, glass, wood, fabric, haze, and reflective floor";
  if (lower.includes("architecture") || lower.includes("interior")) return "a vertical architectural space with furniture, structural lines, natural light, materials, and human scale";
  if (lower.includes("material") || lower.includes("texture")) return "a vertical close-up still life with varied surfaces, fabric, metal, stone, glass, and organic detail";
  if (lower.includes("anime") || lower.includes("manga") || lower.includes("comic") || lower.includes("illustration")) return "a vertical character-and-environment composition with one clear protagonist and readable background";
  if (lower.includes("abstract") || lower.includes("experimental")) return "a vertical gallery-like composition with a central object, motion, light, color fields, texture, and negative space";
  if (lower.includes("food")) return "a vertical tabletop food scene with plate, drink, utensils, fabric, and controlled highlights";
