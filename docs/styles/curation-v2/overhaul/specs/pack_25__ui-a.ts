...
const spec: Spec = {
  ...
  updates: {},
  creates: [
    U('R-UI-01', 'Editorial Index Interface', 'editorial index app layout', 'editorial-index', {
      aesthetic: 'Editorial Index Interface: typography, side indexes and light dividers organize app content with the calm rhythm of a well-set publication.',
      subject_treatment: "Lay out the prompt's app or screen through type hierarchy and a side index, using light dividers instead of repeated cards; keep every requested label exact.",
      color_and_tone: 'Neutral base with one accent reserved for selection and the main action only.',
      lighting_and_shadow: 'Flat screen rendering where contrast and surface hierarchy replace any volumetric shadow.',
      texture_and_material: 'Clean flat surfaces and fine rules, leaving the type to do the talking.',
      camera_and_composition: 'Narrow index column, dominant reading column and secondary detail with its own margin.',
      atmosphere_and_mood: 'Serene editorial rhythm with generous pauses, still clearly a working app.',
      rendering_and_quality: 'Aligned baselines, clear heading spacing and stable reading with long titles.',
    ...
    U('R-UI-02', 'Instrument Rail Interface', 'control-to-reading rail layout', 'instrument-rail', {
      aesthetic: 'Instrument Rail Interface: controls sit on continuous rails that link each one directly to the parameter or result it changes.',
      subject_treatment: "Build the prompt's screen as parallel rails where every control lines up with its own reading, keeping requested labels and values exact.",
      color_and_tone: 'Color by functional role and neutral values for secondary information.',
      lighting_and_shadow: 'Very restrained depth, with active indicators glowing only where they carry function.',
      texture_and_material: 'Sober rails, defined controls and smooth surfaces with no fake brushed metal.',
      camera_and_composition: 'Input, control and result grouped on one axis, rails separated by space rather than frames.',
      atmosphere_and_mood: 'Operational precision with an instrumental feel, without recreating a whole device.',
      rendering_and_quality: 'Every reading is clearly attributed to its control, with no crossing lines.',
    ...
    U('R-UI-03', 'Soft Containment Interface', 'soft container app layout', 'soft-containment', {
      aesthetic: 'Soft Containment Interface: expressive soft-edged containers group tasks by hierarchy with consistent inner spacing.',
      subject_treatment: "Group the prompt's screen into soft-edged containers shaped by hierarchy, one large primary container and compact secondary groups; keep requested labels exact.",
      color_and_tone: 'One neutral main surface and at most two color roles for activity and attention.',
      lighting_and_shadow: 'Soft shadows only between functional levels, with every other surface flat.',
      texture_and_material: 'Matte surfaces and soft edges with related radii throughout.',
      camera_and_composition: 'Large primary container, compact secondary groups and consistent inner spacing.',
      atmosphere_and_mood: 'Closeness and breathing room, expressive in shape but still a serious tool.',
    ...
    U('R-UI-04', 'Ledger-and-Tab Interface', 'ledger rows and index tab layout', 'ledger-tab', {
      aesthetic: 'Ledger-and-Tab Interface: clear rows and index tabs create a register you read like a well-kept ledger, crisp and textureless.',
      subject_treatment: "Build the prompt's records as clear rows under stable headers with index tabs tied to the content; keep every column name and value exact.",
      color_and_tone: 'Neutral background, a very discreet row alternation and one accent for selection.',
      ...
      camera_and_composition: 'Columns aligned by data type, headers visible even in dense layouts.',
      atmosphere_and_mood: 'Archive order with contained warmth, neat and trustworthy.',
      rendering_and_quality: 'Numbers, long labels and empty cells keep their alignment and meaning.',
    ...
    U('R-UI-05', 'Open-Frame Utility', 'partial-border workspace layout', 'open-frame-utility', {
      aesthetic: 'Open-Frame Utility: partial borders, shared guide lines and sober grounds define regions without boxing everything into cards.',
      subject_treatment: "Divide the prompt's workspace with partial borders and shared guides, leaving open the sides that add no grouping; keep requested labels exact.",
      ...
      camera_and_composition: 'Guides align content across open regions so no block needs its own frame.',
      atmosphere_and_mood: 'Technical lightness and one continuous workspace, finished and deliberate.',
    ...
    U('R-UI-06', 'Layered Paper Controls', 'stacked sheet control layout', 'layered-paper', {
      aesthetic: 'Layered Paper Controls: a few overlapping flat sheets with discreet edges show grouping and editing state like tidy index cards.',
      subject_treatment: "Stack the prompt's screen as a few overlapping planes with minimal edges that show groups and edit state; keep requested labels exact.",
      ...
      texture_and_material: 'Smooth graphic paper-like surfaces with crisp edges and no visible fibre.',
    ...
    U('R-UI-07', 'Dense Precision Console', 'dense operational console layout', 'dense-console', {
      aesthetic: 'Dense Precision Console: a short type scale, compact rows and sober states pack a lot of operational information while staying legible.',
      subject_treatment: "Set the prompt's screen with a short type scale, compact rows and functional groups, keeping every label, value and unit exact and readable.",
      color_and_tone: 'Controlled contrast, clear primary reading, discreet helpers and states backed by shape.',
      lighting_and_shadow: 'Flat screen with signal lights only for authorized active states.',
      texture_and_material: 'Neutral surfaces, precise separators and controls of coherent density.',
      camera_and_composition: 'Aligned columns and rows with stable action areas and room for focus.',
      atmosphere_and_mood: 'Professional concentration, dense with information yet calm and readable.',
      rendering_and_quality: 'Full labels at real size, values, units and actions clearly distinct.',
    ...
    U('R-UI-08', 'Chromatic Function Blocks', 'color-coded role block layout', 'chromatic-blocks', {
      aesthetic: 'Chromatic Function Blocks: bold color blocks consistently mark roles and actions, always backed by shape, icon or fixed position.',
      subject_treatment: "Assign the prompt's screen areas to a few recurring color blocks by role, each backed by shape or position; keep requested labels exact.",
      color_and_tone: 'Short palette of strong colors on neutral surfaces, meaning never carried by color alone.',
      ...
      camera_and_composition: 'Each role keeps its place and relative size, with calm reading zones between.',
    ...
    U('R-UI-09', 'Tactile Recessed Controls', 'recessed physical-feel control panel', 'tactile-recessed', {
      aesthetic: 'Tactile Recessed Controls: recessed sliders, grip edges and small status lights make every action feel physical and clear.',
      subject_treatment: "Express the prompt's controls as recessed sliders, grip-edged buttons and small status lights, grouped by action; keep requested labels exact.",
      color_and_tone: 'Neutral dark or light surface with localized accent lights and legible labels.',
      lighting_and_shadow: 'Broad ambient light with moderate cavity shade and small contained LEDs.',
      texture_and_material: 'Sober semi-matte material with low relief and no mandatory chrome.',
      camera_and_composition: 'Controls grouped by action and spaced so every recess reads clearly.',
      atmosphere_and_mood: 'Sober tactility and precision, satisfying to imagine pressing.',
      rendering_and_quality: 'Button, slider and reading stay distinct without photographic realism.',
    ...
    U('R-UI-10', 'Sparse Research Canvas', 'open canvas research layout', 'sparse-canvas', {
      aesthetic: 'Sparse Research Canvas: content pieces and annotations float on a wide calm canvas, joined only by the connections that truly exist.',
      subject_treatment: "Lay the prompt's content out on an open canvas with peripheral notes and only the requested connections, a small inspector appearing on selection; keep labels exact.",
      color_and_tone: 'Calm base, one selection accent and connections lower in contrast than the pieces.',
      ...
      camera_and_composition: 'Wide canvas with room to grow and an inspector only when something is selected.',
