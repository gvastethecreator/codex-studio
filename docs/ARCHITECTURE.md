...
- `components/create/CreateWorkspace.tsx` gives every Create workflow one tool rail, an optional options panel, and a right-hand stage. `RecipeWorkbenchContext` routes controls and the primary action into the rail; recipe pages render in the stage instead of inside the rail. The toolbar owns one controls scroll area and a fixed execution footer.
- Options panels occupy a 300px column from 1280px, overlay the stage at smaller desktop widths, and stay with Configure below 1024px. Escape closes a panel and restores its trigger. Changing workflow unmounts its panel and controls.
...
- Styles separates three state owners: `useStyleBrowserNavigation` owns routes, filters and favorites; `useStyleComposition` owns selected layers and generation inputs; `useUserStyleLibrary` owns catalog reads and editor sessions. The Styles recipe has two views: a compact selector in the Create tray as the primary selection surface, and a demand-mounted options panel for visual browsing. Draft preparation and the editor remain demand-loaded. Late mutation responses update catalog and selected layers only when their version is current, even after the editor closes.
