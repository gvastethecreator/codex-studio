...
- `lib/recipeModuleExamples.ts`: blueprints provider-independent para futuros Recipe Modules. Mantiene ejemplos `sprite_sheet` y `texture_generate` como `example_only`, genera Generation Task Specs sin provider activo y se valida con `recipes:examples:verify`.
...
- `components/recipes/styles/manifests/`: fuente granular de Style Pack Manifests y Style Preset Manifests. `styles/manifestTypes.ts` contiene contratos de authoring/catalogo y `styles/runtimeTypes.ts` contiene contratos runtime de UI/default generation; el viejo barrel `styles/types.ts` fue retirado. `stylePresetCatalogData.ts` carga el grafo editorial completo para validacion y busqueda; `styleRuntimeData.generated.ts` materializa un indice liviano de packs, `styleRuntimePacks.generated/<pack>.ts` compone cada pack desde chunks lazy por categoria en `styleRuntimePacks.generated/<pack>/<category>.ts`, y `stylesData.ts` alimenta la UI sin importar todos los presets al montar. `styles:runtime:check` prueba que esa materializacion siga sincronizada.
- `components/recipes/styleBrowserRenderPlan.ts`: plan de render del Styles Browser compartido por UI y `styles:render`. Mide secciones montadas, secciones eager, placeholders y cards planeadas/eager usando la misma logica que `StylesRecipe`.
...
- `LegacyVisualBatchContext` no expone el snapshot completo. Convierte snapshots legacy en refs `{id, workspaceId}` en el borde de compatibilidad y el reducer conserva solo esos refs para dedupe de recovery y append generado.
...
- Nuevas tareas de asset pipeline se prueban primero como Recipe Module Examples provider-independent; no aparecen en `RecipesView` ni vuelven ejecutable ningun provider hasta tener UI, builder y adapter explicitos.
