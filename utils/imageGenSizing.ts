...
export const IMAGE_GEN_RATIO_OPTIONS: { ratio: AspectRatio; label: string; size: ImageGenSize; width: number; height: number }[] = [
...
export function getImageGenSizeForRatio(ratio: AspectRatio | null | undefined) {
  return IMAGE_GEN_RATIO_OPTIONS.find(option => option.ratio === ratio) || IMAGE_GEN_RATIO_OPTIONS[0];
}
...
export function normalizeImageGenRatio(ratio: string | null | undefined): AspectRatio {
  ...
  if (ratio === '16:9' || ratio === '21:9' || ratio === '4:3' || ratio === '5:4' || ratio === '4:1' || ratio === '8:1') return '3:2';
