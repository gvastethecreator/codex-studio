...
For Grok Imagine, use `apps/local-server/src/providers/grokImagineInput.ts`, `grokImagineExecutor.ts`, and `apps/local-server/src/grokRuntimeDoctor.ts`. Reuse the CLI-owned `grok login`; never add `XAI_API_KEY` to Studio settings. Run one fresh strict headless session per Job with only `image_gen` or `image_edit`, disabled memory/web/plans/subagents, exact one-image validation, and no automatic retry after a possibly billable call. Styles is the current dual-provider recipe and accepts up to five managed local Library images; those references may cross into `image_edit`, while unresolved inline or remote-only assets must fail before execution. Copy generated session media into the Library and leave Grok-owned session history untouched.
