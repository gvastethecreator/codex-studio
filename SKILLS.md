...
Use `apps/local-server/src/providers/externalProvider.ts` as the adapter shell for hosted/local providers. It may compile inputs and fail with preflight diagnostics, but only a concrete executor should make a provider executable. Grok Imagine, Google, fal.ai, and ComfyUI are explicit built-in cases; do not add a runtime registry unless the product gains a real plugin contract.
For Grok Imagine, use `apps/local-server/src/providers/grokImagineInput.ts`, `grokImagineExecutor.ts`, `apps/local-server/src/grokRuntimeDoctor.ts`, and `packages/shared/src/grokImagineContract.ts`. Reuse the CLI-owned `grok login`; never add `XAI_API_KEY` to Studio settings. Parse the live `grok models` list, including `Default model:` and dash bullets. Use the Runtime Doctor default model when `GROK_IMAGE_MODEL` and Settings do not override; omit `--model` when the compiled model is empty. Run one fresh strict headless session per Job with only `image_gen` or `image_edit`, disabled memory/web/plans/subagents, exact one-image validation, and no automatic retry after a possibly billable call. Home and Styles are the current Grok product slice. Styles accepts up to five managed local Library images; those references may cross into `image_edit`. Reject unsupported ratios, missing models, remote-only assets, and non-Grok recipes at job intake and in the Generate dock. Copy generated session media into the Library and leave Grok-owned session history untouched. The verified CLI floor is Grok Build 0.2.114; this checkout is proven against 1.0.4.
