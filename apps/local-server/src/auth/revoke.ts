...
import { resolveGoogleOAuthEndpoint } from './googleOAuthConfig';
...
export async function revokeSubscriptionToken(
  ...
  record: Pick<StoredSubscriptionTokens, 'accessToken' | 'refreshToken'>,
  {
    ...
    timeoutMs = 10_000,
    env = process.env,
  }: RevokeSubscriptionTokenDependencies = {},
  ...
  try {
    let response: Awaited<ReturnType<AuthFetch>>;
    if (providerId === 'codex') {
      response = await fetchImpl(CODEX_OAUTH_REVOKE_URL, {
        method: 'POST',
        headers: {
      ...
      response = await fetchImpl(XAI_OAUTH_REVOKE_URL, {
        method: 'POST',
        headers: {
          Accept: 'application/json',
      ...
      const revokeUrl = resolveGoogleOAuthEndpoint(
