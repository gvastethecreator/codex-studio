                ...
                {serverJobs.length > 0 && (
                    <div className="mb-3 rounded-xl border border-white/10 bg-white/[0.03] p-2">
                        <div className="mb-2 flex items-center justify-between px-1">
                            <span className="text-[9px] font-black uppercase tracking-widest text-white/35">Persistent Backend</span>
...
const ServerJobItem: React.FC<{ job: StudioJob }> = ({ job }) => {
    const isActive = job.status === 'queued' || job.status === 'running' || job.status === 'needs_review';
...
const JobItem: React.FC<{ job: QueueJob, onRetry: () => void, onCancel: () => void, onRemove: () => void }> = ({ job, onRetry, onCancel, onRemove }) => {
    const statusConfig: Record<string, { icon: any, color: string, bg: string, border: string, spin?: boolean }> = {
        pending: { icon: Clock, color: 'text-white/40', bg: 'bg-white/5', border: 'border-white/5' },
        processing: { icon: Loader2, color: 'text-accent-400', bg: 'bg-accent-500/10', border: 'border-accent-500/20', spin: true },
        completed: { icon: CheckCircle2, color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' },
        failed: { icon: XCircle, color: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500/20' },
        cancelled: { icon: XCircle, color: 'text-white/20', bg: 'bg-white/5', border: 'border-white/5' },
    };
