...
const TONE_STYLES: Record<ConfirmationTone, { icon: React.ReactNode; shell: string; button: string }> = {
    danger: {
        icon: <ShieldAlert size={18} />,
        shell: 'bg-rose-500/10 text-rose-300 border border-rose-500/20',
        button: 'bg-rose-500/15 text-rose-100 border border-rose-500/20 hover:bg-rose-500/20 hover:border-rose-400/30',
    },
    warning: {
        icon: <AlertTriangle size={18} />,
        shell: 'bg-amber-500/10 text-amber-300 border border-amber-500/20',
        button: 'bg-amber-500/15 text-amber-100 border border-amber-500/20 hover:bg-amber-500/20 hover:border-amber-400/30',
    },
    accent: {
        icon: <RotateCcw size={18} />,
        shell: 'bg-accent-500/10 text-accent-300 border border-accent-500/20',
        button: 'bg-accent-500/15 text-accent-100 border border-accent-500/20 hover:bg-accent-500/20 hover:border-accent-400/30',
    },
};
                ...
                >
                    <div className="flex items-start justify-between gap-4 border-b border-white/5 p-6 bg-zinc-900/50">
                        <div className="flex min-w-0 items-start gap-3">
                            <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${toneStyles.shell}`}>
                                {toneStyles.icon}
                            </div>
                        ...
                        <div className="border-b border-white/5 px-6 py-5">
                            {details?.length ? (
                                <ul className="space-y-2">
                                    {details.map((detail) => (
                                        <li key={detail} className="flex items-start gap-2 text-[11px] leading-relaxed text-zinc-400">
                            ...
                            {note ? <p className="mt-3 text-[10px] font-bold uppercase tracking-wider text-zinc-500">{note}</p> : null}
                        </div>
                    )}
