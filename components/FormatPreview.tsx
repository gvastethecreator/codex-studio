...
export const FormatPreview: React.FC<FormatPreviewProps> = ({ ratio, isVisible, isWorkspaceEmpty = false }) => {
      ...
      {isVisible && data && (
        ...
        >
          ...
          >
            ...
            <div className={`flex flex-col items-center gap-2 transition-all duration-700 ${isWorkspaceEmpty ? 'opacity-20' : 'opacity-60'}`}>
                ...
                <div className="flex items-center gap-3 px-3 py-1 bg-white/5 border border-white/10 rounded-full">
                   <span className="text-[10px] font-mono font-black text-accent-400 uppercase tracking-widest">{data.width}px</span>
                   <div className="w-1 h-1 bg-white/20 rounded-full" />
                   <span className="text-[10px] font-mono font-black text-accent-400 uppercase tracking-widest">{data.height}px</span>
                </div>
            </div>
            
            {/* Technical Corner Accents */}
            <div className={`absolute top-6 left-6 w-6 h-6 border-t border-l rounded-tl-sm ${isWorkspaceEmpty ? 'border-white/20' : 'border-accent-400'}`} />
            <div className={`absolute top-6 right-6 w-6 h-6 border-t border-r rounded-tr-sm ${isWorkspaceEmpty ? 'border-white/20' : 'border-accent-400'}`} />
            <div className={`absolute bottom-6 left-6 w-6 h-6 border-b border-l rounded-bl-sm ${isWorkspaceEmpty ? 'border-white/20' : 'border-accent-400'}`} />
            <div className={`absolute bottom-6 right-6 w-6 h-6 border-b border-r rounded-br-sm ${isWorkspaceEmpty ? 'border-white/20' : 'border-accent-400'}`} />
            ...
            {!isWorkspaceEmpty && (
                <div className="absolute bottom-10 left-10 flex flex-col items-start gap-1">
                    <span className="text-[7px] font-black text-accent-500 uppercase tracking-[0.3em]">Calibration Active</span>
