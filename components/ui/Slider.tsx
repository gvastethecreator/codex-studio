...
const Slider: React.FC<SliderProps> = ({ label, icon, value, min, max, step = 1, onChange, className = '' }) => {
  ...
  return (
    <div className={`flex flex-col gap-3 ${className}`}>
      <div className="flex justify-between items-center text-xs text-zinc-400 font-medium tracking-wide">
        ...
        <span className="font-mono text-accent-300 bg-accent-950/40 px-1.5 py-0.5 rounded border border-accent-500/20 transition-all">{value}</span>
      </div>
