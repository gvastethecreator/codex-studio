...
export const FloatingTooltip: React.FC<FloatingTooltipProps> = ({ content, children, delay = 300 }) => {
  ...
  return (
    ...
    >
      ...
      {typeof window !== 'undefined' && createPortal(
        <AnimatePresence>
          {isVisible && (
            <TooltipPortal content={content} position={position} />
          )}
        </AnimatePresence>,
        document.body
      )}
...
const TooltipPortal = ({ content, position }: { content: React.ReactNode, position: { x: number, y: number } }) => {
