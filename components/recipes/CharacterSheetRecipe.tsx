...
const STYLE_OPTIONS = ['Preserve Source Style', 'Concept Art (Digital)', 'Anime (90s Retro)', 'Anime (Studio Ghibli)', 'Anime (Modern Kyoto)', 'Comic Book (Western)', 'Graphic Novel (Noir)', '3D Render (Pixar-Style)', '3D Render (Unreal Engine 5)', 'Fantasy Oil Painting', 'Watercolor Illustration', 'Pencil Sketch', 'Technical Blueprint', 'Vector Art (Flat)', 'Pixel Art (High Bit)', 'Cyberpunk Neon'];
const SHOT_OPTIONS = ['Full Body', 'Knee Up', 'Upper Body', 'Portrait (Headshot)', 'Macro Details'];
const FOCUS_OPTIONS = ['General Design', 'Facial Features', 'Outfit & Cloth', 'Anatomy/Muscle', 'Weapons/Gear', 'Hair & Accessories'];
...
export const CharacterSheetRecipe: React.FC<CharacterSheetRecipeProps> = ({
  ...
  useEffect(() => {
    const layoutInstruction = LAYOUT_PROMPTS[params.layout] || LAYOUT_PROMPTS['Classic Turnaround'];
    const styleInstruction = params.style === 'Preserve Source Style' 
        ? "Use the reference image's art style (line weight, shading, color palette) as the main style guide."
        : `RENDER STYLE: ${params.style.toUpperCase()}. Ignore reference image style if it conflicts.`;

    const recipeSchema = {
        task_id: "CHARACTER_DESIGN_SHEET",
        mode: hasReference ? "REFERENCE_GUIDED_CHARACTER_SHEET" : "ORIGINAL_CONCEPT_GENERATION",
        layout_specs: {
            type: params.layout.toUpperCase(),
            shot_framing: params.shot.toUpperCase(),
            detailed_instruction: layoutInstruction,
            view_consistency: "CHARACTER_GUIDED_MATCH",
            background: "NEUTRAL_STUDIO_WHITE_OR_GREY"
        },
        art_direction: {
            style_mode: params.style.toUpperCase(),
            design_focus: params.focus.toUpperCase(),
            instruction: styleInstruction
        },
        directives: [
            `Generate a professional ${params.layout}.`,
            layoutInstruction,
            `Use ${params.shot} framing as the target crop.`,
            `Emphasize and detail elements related to: ${params.focus}.`,
            hasReference 
...
`;
  ...
  }, [params, hasReference, updateConfig]); 
  ...
  const BottomDock = useMemo(() => (
    <>
        <ControlDropdown title="Sheet Type" icon={<Layout size={14} />} label={params.layout} options={Object.keys(LAYOUT_PROMPTS)} onSelect={(v) => setParams(p => ({...p, layout: v}))} activeColor="indigo" />
        <div className="w-px h-8 bg-white/10 mx-1 hidden sm:block" />
        <ControlDropdown title="Framing" icon={<Camera size={14} />} label={params.shot} options={SHOT_OPTIONS} onSelect={(v) => setParams(p => ({...p, shot: v}))} activeColor="indigo" />
        <ControlDropdown title="Detail Focus" icon={<ScanFace size={14} />} label={params.focus} options={FOCUS_OPTIONS} onSelect={(v) => setParams(p => ({...p, focus: v}))} activeColor="indigo" />
        <div className="w-px h-8 bg-white/10 mx-1 hidden sm:block" />
        <ControlDropdown title="Art Style" icon={<Palette size={14} />} label={params.style} options={STYLE_OPTIONS} onSelect={(v) => setParams(p => ({...p, style: v}))} activeColor="indigo" />

        <div className="px-3 hidden xl:block">
                <div className="text-[8px] font-black text-indigo-500 uppercase tracking-widest mb-0.5">Mode</div>
  ...
  return (
    <RecipeLayout isGenerating={isGenerating} bottomDock={BottomDock} className="p-6 md:p-12 pb-48 flex items-center justify-center">
                ...
                {hasReference ? (
                    ...
                    >
                        ...
                        <div className="w-24 h-24 rounded-full bg-zinc-900 border border-white/10 flex items-center justify-center group-hover:scale-110 group-hover:border-indigo-500/50 transition-all shadow-2xl relative z-10">
                            <Brush size={32} className="text-zinc-600 group-hover:text-indigo-400 transition-colors" />
                        </div>
                        <div className="text-center relative z-10">
                            <h3 className="text-2xl font-black text-zinc-300 group-hover:text-white uppercase tracking-tight">Character Source</h3>
