// Generated by scripts/generate-style-runtime-data.ts.
// Edit Style Preset Manifests, then run `bun run styles:runtime`.
import type { StylePack, StylePresetDef } from '../styles/types';

const CATEGORY_PRESET_LOADERS: Array<() => Promise<StylePresetDef[]>> = [
  () => import('./pack_02/film-genres-1').then((module) => module.GENERATED_STYLE_PRESETS),
  () => import('./pack_02/tv-and-broadcast-2').then((module) => module.GENERATED_STYLE_PRESETS),
  () => import('./pack_02/animation-styles-3').then((module) => module.GENERATED_STYLE_PRESETS),
  () =>
    import('./pack_02/lighting-and-atmosphere-4').then((module) => module.GENERATED_STYLE_PRESETS),
  () => import('./pack_02/photography-eras-5').then((module) => module.GENERATED_STYLE_PRESETS),
  () =>
    import('./pack_02/caricature-and-cartoon-styles-6').then(
      (module) => module.GENERATED_STYLE_PRESETS,
    ),
];

export async function loadGeneratedStylePack(): Promise<StylePack> {
  const categoryPresets = await Promise.all(CATEGORY_PRESET_LOADERS.map((loader) => loader()));
  return {
    id: 'pack_02',
    name: 'Cinematic & Media',
    description:
      'A collection of 80+ styles covering film language, broadcast aesthetics, animation traditions, and historical photography.',
    presets: categoryPresets.flat(),
  };
}
