...
import { Upload, Sun, Camera, X, Palette, MonitorPlay, Fingerprint, Type as TextIcon } from 'lucide-react';
...
const CONTROL_OPTIONS = {
  style: ['Reconstrucción Realista', 'Renderizado Cinematográfico', 'Arte Digital Pro', 'Restauración de Archivo', 'Película Analógica', 'Detalle de Óleo'],
  lighting: ['Corrección de Iluminación', 'Luz Volumétrica', 'Iluminación de Estudio', 'Luz Natural', 'Hora Dorada', 'Contraste Dramático'],
  ...
  color: ['Rango Dinámico Ampliado', 'Colores Naturales', 'Vibrancia Profunda', 'Corrección de Color']
};
...
export const RemasterRecipe: React.FC<RemasterRecipeProps> = ({
  ...
  useEffect(() => {
    const adherence = params.fidelity / 100;
    const creativity = (100 - params.fidelity) / 100;

    const remasteredPrompt = `
      ...
      **Core Directives:**
      - **Style Interpretation:** Apply a '${params.style}' aesthetic.
      - **Lighting Correction:** Adjust lighting based on '${params.lighting}'. Favor realistic light falloff and shadow detail.
      - **Anatomical Correction:** ${params.anatomy}. Reduce visible structural inconsistencies or artifacts where possible.
      - **Text Handling:** ${params.text}.
      - **Lens & Detail:** Focus on '${params.camera}'. Enhance micro-contrast and edge sharpness.
      - **Color Grading:** Apply '${params.color}' grading. Balance white levels and enhance tonal range.
    ...
    updateConfig('recipeContext', remasteredPrompt);
  }, [params, updateConfig]);
  ...
  const BottomDock = useMemo(() => (
    <>
        ...
        <div className="flex items-center gap-3 flex-wrap justify-center flex-1">
            <ControlDropdown title="Aesthetic" icon={<MonitorPlay size={14} />} label={params.style} options={CONTROL_OPTIONS.style} onSelect={(v) => setParams(p => ({...p, style: v}))} />
            <ControlDropdown title="Lighting" icon={<Sun size={14} />} label={params.lighting} options={CONTROL_OPTIONS.lighting} onSelect={(v) => setParams(p => ({...p, lighting: v}))} />
            <ControlDropdown title="Correction" icon={<Fingerprint size={14} />} label={params.anatomy} options={CONTROL_OPTIONS.anatomy} onSelect={(v) => setParams(p => ({...p, anatomy: v}))} />
            <ControlDropdown title="Text Handling" icon={<TextIcon size={14} />} label={params.text} options={CONTROL_OPTIONS.text} onSelect={(v) => setParams(p => ({...p, text: v}))} />
            <ControlDropdown title="Color Grading" icon={<Palette size={14} />} label={params.color} options={CONTROL_OPTIONS.color} onSelect={(v) => setParams(p => ({...p, color: v}))} />
            <ControlDropdown title="Lens Details" icon={<Camera size={14} />} label={params.camera} options={CONTROL_OPTIONS.camera} onSelect={(v) => setParams(p => ({...p, camera: v}))} />
        </div>
    </>
  ), [params]);

  return (
    <RecipeLayout isGenerating={isGenerating} bottomDock={BottomDock} className="p-8 pt-20 pb-48 flex items-center justify-center">
        <div 
        ...
        {activeImage ? (
            <div className="w-full h-full relative">
                ...
                <div className="absolute top-4 right-4">
                     <div className="px-3 py-1.5 rounded-lg bg-black/50 backdrop-blur-sm border border-white/10 flex items-center gap-2">
                        <span className="text-[9px] font-black text-zinc-300 uppercase tracking-widest">Remaster Active</span>
                     </div>
                </div>
                <button onClick={() => updateConfig('attachments', [])} className="absolute top-4 left-4 p-2 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white transition-all opacity-0 group-hover:opacity-100"><X size={14} /></button>
            </div>
        ) : (
            <div 
            ...
            >
                ...
                <div className="text-center">
                    <h3 className="text-xl font-black text-zinc-500 group-hover:text-white uppercase tracking-tight">Cargar imagen o escribir un prompt</h3>
                    <p className="text-[10px] text-zinc-700 font-bold uppercase tracking-[0.2em] mt-2">Formatos soportados: JPG, PNG, WEBP</p>
                </div>
            </div>
        )}
        </div>
