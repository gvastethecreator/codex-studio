...
import { Grid3X3, Palette, PaintBucket, Eye, SeparatorHorizontal, Hash, ScanLine, Edit3, X, ChevronLeft } from 'lucide-react';
...
const CONTROL_OPTIONS = {
  view: ['Match Source', 'Isometric', 'Top Down', 'Side Scroll', 'Front View', 'Back View', '3/4 View'],
  style: ['Preserve Style', 'Pixel Art (16-bit)', 'Pixel Art (32-bit)', 'Vector Flat', 'Hand Drawn', 'Voxel', 'Low Poly 3D'],
...
export const SpritesheetRecipe: React.FC<SpritesheetRecipeProps> = ({
  ...
  useEffect(() => {
    ...
    const cellDirectives = Object.entries(cellPrompts)
        .filter(([_, prompt]) => (prompt as string).trim() !== '')
        .map(([index, prompt]) => `Cell ${parseInt(index) + 1}: ${prompt}`);

    const hasDividers = params.dividers !== 'No Dividers';
    const dividerColor = params.dividers.split(' ')[0].toUpperCase();

    let bgDirective = params.background.toUpperCase();
    if (params.background.includes('Green')) bgDirective = 'SOLID_GREEN_#00FF00';
    else if (params.background === 'Custom') bgDirective = `SOLID_COLOR_${customColor.toUpperCase()}`;
    else if (params.background === 'Black') bgDirective = 'SOLID_BLACK';
    ...
    const recipeSchema = {
        ...
        visual_style: {
            perspective: params.view === 'Match Source' ? 'INFER_FROM_SOURCE_IMAGE' : params.view.toUpperCase(),
            rendering: params.style === 'Preserve Style' ? 'SOURCE_CONSISTENT' : params.style.toUpperCase(),
            background_color: bgDirective
        },
        cell_specifics: cellDirectives.length > 0 ? cellDirectives : "Suggest a readable pose or animation cycle (e.g., walk, run, idle, or attack).",
        directives: [
            ...
            params.view === 'Match Source' ? "Use the input image perspective as a guide across all frames." : `Use ${params.view} as the target perspective.`,
...
--- END CONFIG ---
`;
    updateConfig('recipeContext', technicalPrefix);
    return () => { updateConfig('recipeContext', ''); };
  }, [params, cellPrompts, gridCols, gridRows, customColor, updateConfig]);

  const getDividerStyle = () => {
      switch(params.dividers) {
          case 'Red Lines': return 'bg-red-500';
          case 'Blue Lines': return 'bg-blue-500';
          case 'Black Lines': return 'bg-black';
          case 'White Lines': return 'bg-white';
          default: return 'bg-transparent';
      }
  };

  const getBackgroundClass = () => {
      if (params.background === 'Checkerboard') return 'bg-[linear-gradient(45deg,#27272a_25%,transparent_25%,transparent_75%,#27272a_75%,#27272a),linear-gradient(45deg,#27272a_25%,transparent_25%,transparent_75%,#27272a_75%,#27272a)] bg-[length:20px_20px] bg-[position:0_0,10px_10px] bg-zinc-900';
  ...
  const BottomDock = useMemo(() => (
    <>
        <ControlDropdown title="Perspective" icon={<Eye size={14} />} label={params.view} options={CONTROL_OPTIONS.view} onSelect={(v) => setParams(p => ({...p, view: v}))} activeColor="emerald" />
        <ControlDropdown title="Render Style" icon={<Palette size={14} />} label={params.style} options={CONTROL_OPTIONS.style} onSelect={(v) => setParams(p => ({...p, style: v}))} activeColor="emerald" />
        <ControlDropdown title="Layout" icon={<Grid3X3 size={14} />} label={params.grid} options={CONTROL_OPTIONS.grid} onSelect={(v) => setParams(p => ({...p, grid: v}))} activeColor="emerald" />
        <ControlDropdown title="Background" icon={<PaintBucket size={14} />} label={params.background} options={CONTROL_OPTIONS.background} onSelect={(v) => setParams(p => ({...p, background: v}))} activeColor="emerald" />
        {params.background === 'Custom' && (
            <div className="flex flex-col gap-1.5">
                <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest pl-1">Hex</span>
                <MinimalColorPicker color={customColor} onChange={setCustomColor} />
            </div>
        )}
        <ControlDropdown title="Separation" icon={<SeparatorHorizontal size={14} />} label={params.dividers} options={CONTROL_OPTIONS.dividers} onSelect={(v) => setParams(p => ({...p, dividers: v}))} activeColor="emerald" />
        <div className="w-px h-8 bg-white/10 mx-2 hidden sm:block" />
        <div className="px-2 hidden sm:block">
                <div className="text-[8px] font-black text-emerald-500 uppercase tracking-widest mb-0.5">Status</div>
         ...
         <div className="flex w-full h-full gap-6 items-center justify-center relative">
             ...
             <div className="flex-1 flex flex-col items-center justify-center relative min-w-0 h-full">
                 ...
                 >
                     {Array.from({ length: gridCols * gridRows }).map((_, i) => (
                         <div 
                            ...
                            style={{ backgroundColor: params.background === 'Custom' ? customColor : undefined }}
                             ...
                             {editingCell === i ? (
                                <div className="absolute inset-0 z-20 bg-zinc-900/95 flex flex-col p-1 animate-in fade-in zoom-in-95 duration-200">
                                 ...
                                 <div className="absolute inset-0 p-1 flex flex-col items-center justify-center opacity-60 group-hover:opacity-100 transition-opacity">
                                     <span className={`text-[10px] font-black mb-0.5 drop-shadow-md ${isLightBg ? 'text-black/50' : 'text-white/30'}`}>
                                         {i + 1}
                                     </span>
                                     {cellPrompts[i] && (
                                         <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(var(--emerald-500),0.8)]" />
                                     )}
                                     {hoveredCell === i && !cellPrompts[i] && <Edit3 size={12} className={isLightBg ? 'text-black/30' : 'text-white/30'} />}
                                 </div>
                             )}
                         </div>
                     ))}
                 ...
                 <div className="absolute -bottom-12 bg-black/60 border border-white/10 px-4 py-2 rounded-xl flex items-center gap-3 shadow-lg pointer-events-none">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-white uppercase tracking-widest">{config.aspectRatio} Canvas</span>
                        <span className="text-[8px] font-bold text-zinc-500 uppercase">{gridCols}x{gridRows} Grid • {params.dividers}</span>
                    </div>
                 </div>
             </div>
             ...
             `}>
                  <div className="h-14 border-b border-white/5 flex items-center px-5 gap-2 bg-white/[0.02]">
                      ...
                      <span className="text-[10px] font-black text-white uppercase tracking-widest">Edit Cells</span>
                      <button onClick={() => setIsSidebarOpen(false)} className="ml-auto text-zinc-500 hover:text-white"><X size={14} /></button>
                  </div>
                      ...
                      {Array.from({ length: gridCols * gridRows }).map((_, i) => (
                          ...
                          >
                              <div className="flex items-center justify-between mb-1.5">
                                  <span className={`text-[9px] font-black uppercase tracking-widest ${hoveredCell === i || editingCell === i ? 'text-emerald-400' : 'text-zinc-600'}`}>
                                      Cell {i + 1}
                                  </span>
                                  {cellPrompts[i] && (
             ...
             {!isSidebarOpen && (
                 <button onClick={() => setIsSidebarOpen(true)} className="absolute right-0 top-1/2 -translate-y-1/2 p-2 bg-zinc-900 border border-white/10 rounded-l-xl hover:bg-zinc-800 text-zinc-400 hover:text-white transition-all shadow-lg"><ChevronLeft size={16} /></button>
             )}
         </div>
    </RecipeLayout>
  );
