...
export const CinematicRecipe: React.FC<CinematicRecipeProps> = ({
  ...
  useEffect(() => {
    const layoutDescription = `${gridLayout.rows} rows by ${gridLayout.cols} columns`;
    let layoutInstruction = "";
    if (params.frames === 3) layoutInstruction = `Create a cinematic triptych (${layoutDescription}).`;
    else if (params.frames === 6) layoutInstruction = `Create a 6-frame storyboard grid (${layoutDescription}).`;
    else layoutInstruction = `Create a 9-frame storyboard contact sheet (3x3 grid).`;

    const frameDirectives = Array.from({length: params.frames}).map((_, i) => {
        const shot = frameShots[i] || 'Auto';
        return shot !== 'Auto' ? `- Frame ${i + 1}: ${shot} Shot` : null;
    }).filter(Boolean);
...
- Genre: ${params.genre}
- Tone: ${params.tone}
- Lighting: ${params.lighting} (Keep light direction and quality coherent across frames where possible)
- Time: ${params.time}
- Weather: ${params.weather}
- Camera Movement: ${params.movement} (Use this as the visual camera language across frames)
- Lens/Focal Length: ${params.lens} (Apply appropriate depth of field and distortion)${frameInstructions}
...
--- END CONFIG ---
`;
    updateConfig('recipeContext', directorPrompt);
  }, [params, gridLayout, config.aspectRatio, updateConfig, frameShots]); 
