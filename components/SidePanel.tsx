...
export const SidePanel: React.FC<SidePanelProps> = ({ position, label = "DASHBOARD", children }) => {
  ...
  return (
    ...
    >
      ...
      {/* Footer Decorativo */}
      <div className={`h-10 w-full flex items-center justify-center opacity-20 transition-all duration-300 ${isCollapsed ? 'opacity-0' : 'border-t border-white/5'}`}>
          <div className={`h-0.5 bg-zinc-800 rounded-full transition-all duration-500 ${isCollapsed ? 'w-0' : 'w-16'}`} />
      </div>
    </aside>
  );
};
