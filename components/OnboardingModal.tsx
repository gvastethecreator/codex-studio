...
import { Activity, ArrowRight, ExternalLink, HardDrive, Play, RefreshCw, Sparkles, Terminal, X } from 'lucide-react';
        ...
        <div className={`rounded-2xl border p-4 ${toneClasses[tone]}`}>
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-2 wrap-break-word font-mono text-[11px] leading-relaxed text-current/85">{detail}</p>
                </div>
            </div>
        </div>
    );
                ...
                >
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                <div className="flex flex-wrap items-center gap-2">
                                    <h2 className="text-lg font-black uppercase tracking-widest text-white md:text-xl">Primer arranque</h2>
                                ...
                                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-400">
                                    Verifica que el studio local pueda hablar con Codex, que la biblioteca este accesible y que el runtime actual quede listo para una futura build Electron sin reescribir el renderer.
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="grid gap-6 p-6 md:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.9fr)] md:px-8 md:py-7">
                        <div className="space-y-4">
                            {error && (
                                <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4">
                                    <p className="text-[11px] font-black uppercase tracking-widest text-red-300">No pude consultar el backend local</p>
                            ...
                            <div className="grid gap-3">
                                <StepRow
                                    ...
                                    detail={backendReachable ? `Disponible en ${apiBase}` : `Pendiente: necesito poder consultar ${apiBase}`}
                                />
                                <StepRow
                                    icon={<Terminal size={16} />}
                                    title="Codex CLI"
                                    tone={!backendReachable ? 'pending' : health?.codexCli.available ? 'ready' : 'warning'}
                                    detail={!backendReachable ? 'Primero necesito alcanzar el backend local.' : health?.codexCli.available ? `Detectado: ${health.codexCli.version || 'version no informada'} · ${health.codexCli.command}` : `Instala o reautentica Codex en esta maquina. Intento actual: ${health?.codexCli.command || 'codex --version'}`}
                                />
                                <StepRow
                                    icon={<Activity size={16} />}
                                    title="codex app-server"
                                    tone={!backendReachable ? 'pending' : health?.appServer.running ? 'ready' : 'warning'}
                        ...
                        <div className="space-y-4">
                            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                                <p className="text-[11px] font-black uppercase tracking-widest text-zinc-500">Checklist rapido</p>
                            ...
                            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                                <p className="text-[11px] font-black uppercase tracking-widest text-zinc-500">Siguiente paso</p>
                            ...
                            {health && (
                                <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                                    <p className="text-[11px] font-black uppercase tracking-widest text-zinc-500">Runtime actual</p>
                                    <div className="mt-3 space-y-2 font-mono text-[11px] leading-relaxed text-zinc-300">
                                        ...
                                        <div className="wrap-break-word">Env local: {health.runtime.envLocalPresent ? health.runtime.envLocalPath : `faltante (${health.runtime.envLocalPath})`}</div>
                                        <div>Puertos: HTTP {health.config.serverPort} · WS {health.config.codexWsPort}</div>
                                        <div>Runtime: Bun {health.runtime.bunVersion || 'desconocida'} · Node {health.runtime.nodeVersion}</div>
                                        <div>{health.runtime.platform} · {health.runtime.arch}</div>
                                    </div>
                                </div>
                            )}

                            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                                <p className="text-[11px] font-black uppercase tracking-widest text-zinc-500">Pensando en Electron</p>
                                <p className="mt-3 text-sm leading-relaxed text-zinc-300">
                                    El renderer ya puede resolver su API base desde runtime en lugar de asumir siempre `localhost`. Eso nos deja una costura limpia para que un preload de Electron inyecte configuracion sin abrir la puerta a Node en la UI.
                                </p>
                                <a
                    ...
                    <div className="flex flex-col gap-3 border-t border-white/5 bg-black/20 px-6 py-5 md:flex-row md:items-center md:justify-between md:px-8">
                        <div className="text-xs text-zinc-500">
                            {isChecking ? 'Actualizando estado del studio...' : isReady ? 'Estado listo para generar.' : 'Aun faltan checks antes de la primera generacion.'}
                        </div>
