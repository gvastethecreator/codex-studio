...
export const ImageEditorModal: React.FC<ImageEditorModalProps> = ({ isOpen, onClose, image, onGenerate, isGenerating }) => {
  ...
  const startDrawing = (e: React.MouseEvent) => {
    ...
    ctx.fillStyle = 'white';
    ctx.beginPath();
    ctx.arc(x, y, (brushSize * (canvasRef.current!.width / canvasRef.current!.getBoundingClientRect().width)) / 2, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.moveTo(x, y);
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/98 backdrop-blur-3xl flex flex-col animate-in fade-in duration-500" onClick={onClose}>
      
      <div className="h-20 w-full flex items-center justify-between px-10 border-b border-white/5" onClick={e => e.stopPropagation()}>
         <div className="flex items-center gap-5">
             <div className="p-2.5 bg-accent-500/10 rounded-xl"><Sparkles size={20} className="text-accent-400" /></div>
             <div>
                <h2 className="text-sm font-black tracking-widest uppercase text-white">Precision Inpaint</h2>
                <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-tight">Regenerate Archives</p>
             </div>
         </div>
         <button onClick={onClose} className="p-3 bg-zinc-900/60 hover:bg-zinc-800 rounded-xl text-zinc-600 hover:text-white transition-all shadow-xl">
            <X size={24} />
         </button>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex-1 relative flex items-center justify-center p-8 bg-[#010101]" ref={containerRef}>
            {image && (
                <img 
                    src={image.dataUrl} 
          ...
          <div className="w-full md:w-96 bg-zinc-950 p-8 flex flex-col gap-10 shadow-[20px_0_60px_rgba(0,0,0,1)]">
              <div className="space-y-4">
                  <label className="text-[10px] font-black text-zinc-700 uppercase tracking-widest">Modification Prompt</label>
                  <textarea
                    ref={textareaRef}
                    autoFocus
                    value={editPrompt}
              ...
              <div className="mt-auto">
                  ...
                  >
                    {isGenerating ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <Sparkles size={18} />}
