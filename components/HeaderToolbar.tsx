...
import { Trash2, X, Plus, ArrowLeft, Home, Bug } from 'lucide-react';
...
export const HeaderToolbar: React.FC<HeaderToolbarProps> = ({ 
  ...
  React.useEffect(() => {
      const handleOutsideClick = (event: MouseEvent) => {
          if (editingWorkspaceId && workspacesContainerRef.current && !workspacesContainerRef.current.contains(event.target as Node)) {
  ...
  return (
    <TopToolbar className="w-full h-14 bg-black/80 backdrop-blur-sm flex items-center px-6 z-40 flex-shrink-0 border-b border-white/5">
      <div className="w-full max-w-[1920px] mx-auto flex items-center justify-between gap-4 relative z-50">
        
        <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
                <Logo isGenerating={isGenerating} />
                <Tooltip content="Debug Panel" position="bottom">
                    <button onClick={onToggleDebug} className="p-1.5 rounded-lg text-zinc-500 hover:text-white hover:bg-white/5 transition-colors cursor-pointer">
                        <Bug size={16} />
                    </button>
                </Tooltip>
            ...
            {/* Workspaces */}
            <div ref={workspacesContainerRef} className="vt-workspace-list hidden md:flex items-center gap-2 px-2">
                {workspaces.map((ws, index) => {
                    ...
                    return (
                        <div key={ws.id} className="relative group flex-shrink-0">
                            <Tooltip content={`Workspace ${ws.name || label}: ${new Date(ws.createdAt).toLocaleDateString()}`} position="bottom">
                                <button 
                                    onClick={() => {
                                        if (isActive) {
                ...
                <Tooltip content="Synthesize New Workspace" position="bottom">
                    <button onClick={onAddWorkspace} className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 hover:bg-accent-500/20 text-zinc-600 border border-dashed border-white/10 transition-all cursor-pointer"><Plus size={18} /></button>
                </Tooltip>
            </div>
        </div>
