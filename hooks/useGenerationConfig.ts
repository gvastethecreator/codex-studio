...
import type { ImageGenerationConfig, Attachment, GeneratedImageWithConfig, AspectRatio, GenerationModel } from '../types';
...
export const useGenerationConfig = ({ log }: UseGenerationConfigProps) => {
    const [generationConfig, setGenerationConfig] = useIndexedDBStorage<ImageGenerationConfig>('generation-config', DEFAULT_GENERATION_CONFIG);
    ...
    const updateGenerationConfig = useCallback(<K extends keyof ImageGenerationConfig>(key: K, value: ImageGenerationConfig[K]) => {
        setGenerationConfig(prev => ({ ...prev, [key]: value }));
    }, [setGenerationConfig]);
    
    const updateAttachment = useCallback((id: string, newProps: Partial<Attachment>) => {
      setGenerationConfig(prev => ({
        ...prev,
    ...
    }, [maxAttachments, generationConfig.attachments.length, generationConfig.model, log, setGenerationConfig]);
    ...
    const handleAddToContext = useCallback((image: GeneratedImageWithConfig) => {
        setGenerationConfig(prev => {
