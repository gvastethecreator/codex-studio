...
interface UseQueueManagerProps {
    executeGeneration: (config: Partial<ImageGenerationConfig>, options?: { preventModal?: boolean; signal?: AbortSignal }) => Promise<void>;
...
export const useQueueManager = ({ executeGeneration, isGenerating, addToast }: UseQueueManagerProps) => {
    ...
    const enqueue = useCallback((prompt: string, config: ImageGenerationConfig, force: boolean = false) => {
        startViewTransition(() => {
    ...
    const clearCompleted = useCallback(() => {
        startViewTransition(() => {
            setJobs(prev => prev.filter(job => job.status !== 'completed' && job.status !== 'cancelled'));
        });
    }, []);
    ...
    useEffect(() => {
        ...
        jobsToStart.forEach(async (nextJob) => {
            ...
            setJobs(prev => prev.map(j => j.id === nextJob.id ? { ...j, status: 'processing' } : j));
            ...
            try {
                ...
                const isAbort = error.name === 'AbortError' || error.message === 'Operation cancelled by user';
